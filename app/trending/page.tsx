...
export default function Trending() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      const { data } = await supabase.from("boostedPosts").select("*").order("boostCount", { ascending: false })
      setPosts(data || [])
    }
    fetchPosts()
  }, [])
  ...
  return (
    <main style={{ padding: 20, textAlign: "center", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 20 }}><a href="/">Back to Boost</a></div>
      ...
      {posts.map((post, index) => (
        <div key={index} style={{ border: "1px solid #ccc", padding: 20, marginTop: 20 }}>
          ...
          <div style={{ marginTop: 10 }}>
            <button onClick={() => window.open(post.link, "_blank")} style={{ padding: "8px 15px", cursor: "pointer" }}>View Post</button>
          </div>
          {post.contract && (
            <div style={{ marginTop: 20 }}>
              <iframe src={`https://dexscreener.com/base/${post.contract}`} width="100%" height="500" style={{ border: "none" }}></iframe>
            </div>
          )}
        </div>
      ))}
