...
export default function BoostForm() {
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "500px" }}>
      ...
      <input placeholder="Post URL" value={postUrl} onChange={(e) => setPostUrl(e.target.value)} style={{ width: "100%", padding: "8px", marginBottom: "15px" }} />
      <div style={{ display: "flex", gap: "5px", marginBottom: "15px" }}>
        {tiers.map((t, i) => (
          <div key={i} onClick={() => { setTierIndex(i); setDurationIndex(0) }}
            style={{ flex: 1, padding: "10px", border: tierIndex === i ? "2px solid #4f46e5" : "1px solid #ccc", borderRadius: "8px", cursor: "pointer" }}>
            <h4>{t.name}</h4>
            <p style={{ fontSize: "12px" }}>{t.description}</p>
            <ul style={{ fontSize: "12px" }}>{t.durations.map((d, idx) => <li key={idx}>{d.label} – {d.price} ETH</li>)}</ul>
          </div>
        ))}
      </div>
      <select value={durationIndex} onChange={(e) => setDurationIndex(Number(e.target.value))} style={{ width: "100%", padding: "8px", marginBottom: "15px" }}>
        {selectedTier.durations.map((d, idx) => <option key={idx} value={idx}>{d.label} – {d.price} ETH</option>)}
      </select>
      <button onClick={handleBoost} style={{ width: "100%", padding: "12px", background: "#4f46e5", color: "#fff", borderRadius: "6px", fontWeight: "bold", cursor: "pointer" }}>
        Boost Now 🚀
      </button>
    </div>
  )
